...
class Config:
    def __init__(self):
        ...
        # (set a DHCP reservation so it never moves). If you ever do change the
...
{bar}
  ...
  What to do:
    1. Find your thermostat's IP address (set a DHCP reservation for it)
    2. In docker-compose.yml remove the Moovair account variables and add:
         M2M_THERMOSTAT_HOST: "192.168.x.x:5555"
    3. docker compose up -d

  Prefer to stay on the cloud version?  Pin the image to  :v2
...
class Bridge:
    ...
    def publish_discovery(self):
        ...
        climate = {
            ...
            # ⚠ °F is UNTESTED — no Fahrenheit hardware available.
